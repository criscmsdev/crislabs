...
import { CreateSite, ListSite, Site } from '@/src/interface/site.interface';
import { hardwareStoreCreateSite } from '@/lib/hardware-store/site/createSite';
interface Error {
...
export const SiteForm: FC<SiteForm> = ({ site }) => {
  ...
  const data = queryClient.getQueryData([
    'wear-get-sites-with-cursor',
    connectionArgs,
  ]);
  // console.log('data', data);
  // 
  const { mutate: createHardwareStoreSite, isLoading } = useMutation({
    mutationFn: 
    
    async (inputCreate: CreateSite) => await hardwareStoreCreateSite(inputCreate),
    onSuccess: async (inputCreate, updateSite, id) => {
      
      // queryClient.setQueryData<ListSite>(
      //   ['wear-get-sites-with-cursor', connectionArgs],
      //   (old) => ({
      //     page: {
      //       edges: old!.page.edges.map((data) => ({
      //         cursor: "",
      //         node: inputCreate,
      //       })),
      //       pageInfo: {
  ...
  //   mutationFn: async (inputUpdate: UpdateSite) =>
  //     await wearUpdateSite(inputUpdate),
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    ...
    if (site) {
      if (query[2] === 'wear') {
        // updateWearSite(updateForm);
      }
    } else {
      if (query[2] === 'hardware-store') createHardwareStoreSite(createForm);
    }
  };
