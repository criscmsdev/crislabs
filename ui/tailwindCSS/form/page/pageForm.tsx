import { hardwareStoreCreatePage0 } from '@/lib/hardware-store/page/page0/createPage';
import { hardwareStoreCreatePage1 } from '@/lib/hardware-store/page/page1/createPage';
import { hardwareStoreCreatePage2 } from '@/lib/hardware-store/page/page2/createPage';
import { useSearch } from '@/src/context/SearchContext';
import { useUI } from '@/src/context/UIContext';
import { CreatePage, ListPage, Page } from '@/src/interface/page.interface';
...
export const PageForm: FC<PageForm> = ({ page, type }) => {
  ...
  const { mutate: createHardwareStorePage0 } = useMutation({
    mutationFn: async (inputCreate: CreatePage) =>
      await hardwareStoreCreatePage0(inputCreate),
    onSuccess: async ({ parentId }) => {
      queryClient.invalidateQueries<ListPage>({
        queryKey: [
          'hardware-store-get-pages0-with-cursor',
          connectionArgs,
          parentId,
        ],
      });
  ...
  const { mutate: createHardwareStorePage1 } = useMutation({
    mutationFn: async (inputCreate: CreatePage) =>
      await hardwareStoreCreatePage1(inputCreate),
    onSuccess: async ({ parentId }) => {
      queryClient.invalidateQueries<ListPage>({
        queryKey: [
  ...
  const { mutate: createHardwareStorePage2 } = useMutation({
    mutationFn: async (inputCreate: CreatePage) =>
      await hardwareStoreCreatePage2(inputCreate),
    onSuccess: async ({ parentId }) => {
      queryClient.invalidateQueries<ListPage>({
        queryKey: [
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    ...
    if (page) {
      ...
      if (query.length === 4 && query[2] === 'hardware-store')
        createHardwareStorePage0({ ...form, parentId: query[3] });
      if (
        query.length === 6 &&
        query[2] === 'hardware-store' &&
        query[4] === 'page0'
      )
        createHardwareStorePage1({ ...form, parentId: query[5] });
      if (
        query.length === 6 &&
        query[2] === 'hardware-store' &&
        query[4] === 'page1'
      )
        createHardwareStorePage2({ ...form, parentId: query[5] });
