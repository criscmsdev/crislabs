...
import { hardwareStoreDeleteSites } from '@/lib/hardware-store/site/deleteSites';
...
export function HeadingDashboardOption() {
  ...
  const { mutate: deleteSitesHardwareStore } = useMutation(
    {
      mutationFn: async (ids: string[]) => await hardwareStoreDeleteSites(ids),
      onSuccess:  (deleteSitesMarketing) => {
        queryClient.invalidateQueries<ListSite>({queryKey: ["hardware-store-get-sites-with-cursor", connectionArgs]});
        Swal.fire({
          title: 'Deleted!',
  ...
  const deleteHandle = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        if (query[2] === 'hardware-store') {
          {
            query.length === 3 && deleteSitesHardwareStore(selected);
          }
          {
